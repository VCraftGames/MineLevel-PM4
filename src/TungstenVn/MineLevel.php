    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool
    {
        if ($sender instanceof Player) {
           if(!isset($args[0])){
               ...
               $sender->sendMessage("§a§l【 §fLevel Thẻ Mùa của bạn hiện tại là §6$level §a】");
               ...
               if (null != $player) {
                   ...
                   $name = $player->getName();
                   $sender->sendMessage("§a§l【 §fLevel thẻ mùa của §6[$name] §flà §6$level §a】");
               } else {
    ...
    public function onBreak(BlockBreakEvent $e)
    {
        ...
        if ($marker != null) {
            if ($block + 1 >= $marker[$level + 1]) {
                ...
                $block = $block + 1;
                if(!isset($marker[$level +1])){
                    $this->getServer()->broadcastMessage("§3§l✉ §c§l[§bVCraftStudios§c] §r§fChúc mừng §e[$name] §fđã §cđạt level cao nhất §fcủa §cMineLevel (thẻ mùa) §f. Level: §e[$level]§f, Tổng block đã đào:§e [$block]");
                }else{
                    $this->getServer()->broadcastMessage("§3§l✉ §c§l[§bVCraftStudios§c] §r§fChúc mừng §e[$name] §fđã lên MineLevel (thẻ mùa) là §e[$level]§f, Tổng block đã đào:§e [$block]");
                }
            }
        } else {
